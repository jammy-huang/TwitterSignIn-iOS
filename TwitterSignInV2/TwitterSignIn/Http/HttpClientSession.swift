    ...
    public func httpRequestHandler(httpMethod:HttpRequest.HTTP_METHOD, urlAddress:String, header:[String:String]?, body:Any?, callBack:@escaping HttpSessionCallback) {
        ...
        if (httpMethod == .get)
        {
            ...
            if let body = body {
                if let bodyDic:[String:Any] = body as? [String:Any] {
                    if (bodyDic.count > 0) {
                        var bodyQueryList:[String] = []
                        bodyDic.forEach { (key, value) in
                            bodyQueryList.append("\(key)=\(value)")
                        }
                        let bodyQueryStr = bodyQueryList.joined(separator: "&")
                        request.httpBody = bodyQueryStr.data(using: String.Encoding.utf8);
                    }
                } else if let bodyArr:[[String:Any]] = body as? [[String:Any]] {
                    if (bodyArr.count > 0) {
                        var bodyQueryList:[String] = []
                        bodyArr.forEach { item in
